...
export default function BrowserScreensaver({ isOpen, onClose }: BrowserScreensaverProps) {
  ...
  // 2. Auto-save on mount, and any keydown to close screensaver
  useEffect(() => {
    if (isOpen) {
      // Trigger global save
  ...
  return (
    <div className="fixed inset-0 z-[9999] overflow-hidden bg-black select-none">
      {/* 1. Zooming Bing Image Background */}
      <motion.div
      ...
      >
        <motion.img
          src="/api/media/screensaver/bing"
          alt="Bing Wallpaper"
      ...
      <button
        ...
        className="absolute top-6 right-6 z-50 p-2.5 rounded-full bg-black/40 hover:bg-black/75 border border-white/10 hover:border-white/20 text-white/70 hover:text-white transition-all shadow-lg hover:scale-105"
        title="关闭屏保"
      >
        <X size={20} />
      </button>

      {/* 4. Center Clock Content */}
