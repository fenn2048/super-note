 ...
 *     "super:new-note"，让 App 层监听。
 *   - "搜索 / 设置" 等还没有全局 open state，暂用自定义事件 "super:open-search" /
 *     "super:open-settings"，后续由对应组件订阅。
...
export interface DesktopMenuBridgeOptions {
  ...
  /** 格式菜单（macOS 格式菜单 / 快捷键）。若未提供则派发 window 事件 "super:format"。 */
  ...
  useEffect(() => {
    ...
    unsubs.push(
      onMenuAction("menu:new-note", () => {
        ...
        else window.dispatchEvent(new CustomEvent("super:new-note"));
      })
    );
    ...
    unsubs.push(
      onMenuAction("menu:search", () => {
        ...
        else window.dispatchEvent(new CustomEvent("super:open-search"));
      })
    );
    unsubs.push(
    ...
    unsubs.push(
      onMenuAction("menu:open-settings", () => {
        ...
        else window.dispatchEvent(new CustomEvent("super:open-settings"));
      })
    );
    unsubs.push(
      onMenuAction("menu:toggle-sidebar", () => {
        if (opts.onToggleSidebar) opts.onToggleSidebar();
        else window.dispatchEvent(new CustomEvent("super:toggle-sidebar"));
      })
    );
    unsubs.push(
      onMenuAction("menu:focus-note-list", () => {
        if (opts.onFocusNoteList) opts.onFocusNoteList();
        else window.dispatchEvent(new CustomEvent("super:focus-note-list"));
      })
    );
      ...
      onFormatMenu((payload) => {
        if (opts.onFormat) opts.onFormat(payload);
          ...
          window.dispatchEvent(
            new CustomEvent<FormatMenuPayload>("super:format", { detail: payload })
          );
      })
    );
    ...
    unsubs.push(
      onOpenFile((file) => {
        if (opts.onOpenFile) opts.onOpenFile(file);
          ...
          window.dispatchEvent(
            new CustomEvent<OpenFilePayload>("super:open-file", { detail: file })
          );
      })
    );
