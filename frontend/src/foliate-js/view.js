...
export class View extends HTMLElement {
    ...
    // Extract the element ID from a CFI value string (handles NOTE_PREFIX too).
    // Returns null if no element ID found in the indir path.
    static #getCfiElementId(cfiValue) {
        const raw = cfiValue.startsWith('foliate-note:')
            ? cfiValue.slice('foliate-note:'.length)
            : cfiValue
        const indirIdx = raw.indexOf('!')
        if (indirIdx === -1) return null
        const indir = raw.slice(indirIdx)
        // Match /N[elementId] patterns in the indir path (skip numeric-only assertions)
        for (const m of indir.matchAll(/\/\d+\[([^\]]+)\]/g)) {
            const id = m[1]
            if (!/^\d/.test(id)) return id  // skip numeric fragment assertions
        }
        return null
    }
    // Try to find the annotation in other loaded sections when the primary section
    // either doesn't have an overlayer or doesn't contain the target element.
    ...
    #tryFallbackDraw(annotation, anchor, primaryIndex, value, remove) {
        ...
        for (const altContent of allContents) {
            ...
            if (!altContent.overlayer || !altContent.doc) continue
            // Strict: only draw in a section that actually has the target element
            if (elementId && !altContent.doc.getElementById(elementId)) continue
            try {
                const altRange = anchor(altContent.doc)
                if (altRange) {
                    console.log('[FoliateView debug] annotation fallback found in index:', altContent.index, 'elementId:', elementId)
                    const altDraw = (func, opts) => altContent.overlayer.add(value, altRange, func, opts)
                    this.#emit('draw-annotation', { draw: altDraw, annotation, doc: altContent.doc, range: altRange })
                    return
                }
            } catch (e) { /* ignore */ }
        }
    ...
    async addAnnotation(annotation, remove) {
        ...
        console.log("[FoliateView debug] addAnnotation called:", { value, remove });
        if (value.startsWith(SEARCH_PREFIX)) {
            const cfi = value.replace(SEARCH_PREFIX, '')
            const { index, anchor } = await this.resolveNavigation(cfi)
            const obj = this.#getOverlayer(index)
            console.log("[FoliateView debug] search prefix:", { index, hasOverlayer: !!obj });
            if (obj) {
            ...
            const { index, anchor } = await this.resolveNavigation(cfi)
            const obj = this.#getOverlayer(index)
            console.log("[FoliateView debug] note prefix:", { index, hasOverlayer: !!obj });
            if (obj) {
                const { overlayer, doc } = obj
                if (remove) {
                    overlayer.remove(value)
                ...
                let range = doc ? anchor(doc) : anchor
                console.log("[FoliateView debug] note prefix range:", { hasRange: !!range });
                if (range) {
                    const draw = (func, opts) => overlayer.add(value, range, func, opts)
                    this.#emit('draw-annotation', { draw, annotation, doc, range })
                } else {
                    this.#tryFallbackDraw(annotation, anchor, index, value, remove)
                }
            } else {
                this.#tryFallbackDraw(annotation, anchor, index, value, remove)
            }
        ...
        const obj = this.#getOverlayer(index)
        console.log("[FoliateView debug] standard prefix:", { index, hasOverlayer: !!obj });
        if (obj) {
            const { overlayer, doc } = obj
            overlayer.remove(value)
            if (!remove) {
                let range = doc ? anchor(doc) : anchor
                console.log("[FoliateView debug] standard range:", { hasRange: !!range });
                if (range) {
                    const draw = (func, opts) => overlayer.add(value, range, func, opts)
                    this.#emit('draw-annotation', { draw, annotation, doc, range })
                } else {
    ...
    #createOverlayer({ doc, index }) {
        ...
        setTimeout(() => this.#emit('create-overlay', { index }), 0)
        return overlayer
    }
    ...
    resolveCFI(cfi) {
        console.log("[FoliateView debug] resolveCFI called with:", cfi);
        try {
            if (this.book.resolveCFI) {
                const res = this.book.resolveCFI(cfi);
                console.log("[FoliateView debug] book.resolveCFI returned:", { index: res?.index, hasAnchor: !!res?.anchor });
                return res;
            } else {
                const parts = CFI.parse(cfi)
                const index = CFI.fake.toIndex((parts.parent ?? parts).shift())
                const anchor = doc => CFI.toRange(doc, parts)
                console.log("[FoliateView debug] fallback resolveCFI resolved index:", index);
                return { index, anchor }
            }
        } catch (err) {
            console.error("[FoliateView debug] resolveCFI failed error:", err);
            throw err;
        }
    }
