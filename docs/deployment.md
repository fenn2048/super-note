...
本文档汇总 super-note 各种环境下的安装与部署方式，包括本地开发、Docker、桌面端 / 移动端、以及主流 NAS 平台。
...
数据库文件位于 `backend/data/super-note.db`，备份此文件即可迁移数据。
...
# 2. 创建数据目录并运行
mkdir -p /opt/super-note/data
docker run -d \
  --name super-note \
  --restart unless-stopped \
  -p 3001:3001 \
  -v /opt/super-note/data:/app/data \
  -e DB_PATH=/app/data/super-note.db \
  super-note
```
...
| 端口 | 服务 | 说明 |
|------|------|------|
| `3001` | super-note | 主应用（前后端一体 + SQLite） |
...
打包产物输出到 `release/` 目录。桌面端自动 fork 后端进程，数据库存储在用户目录 `super-data/` 下。
...
直接从 [GitHub Releases](https://github.com/cropflre/super-note/releases) 下载最新的 APK 安装包，传输到手机安装即可。
...
移动端首次启动需配置服务器地址（IP:端口 或域名），通过 HTTP 连接到已部署的 super-note 后端。
...
1. **上传镜像**
   - 在电脑上执行 `docker build -t super-note .` 构建镜像
   - 导出镜像：`docker save super-note -o super-note.tar`
   - 在群晖 Container Manager → 映像 → 导入 → 上传 `super-note.tar`

2. **创建容器**
   - 映像列表中选择 `super-note` → 启动
   - **端口设置**：本地端口 `3001` → 容器端口 `3001`
   - **存储空间**：新增文件夹映射
     - 本地路径：`/docker/super-note/data`
     - 容器路径：`/app/data`
   - **环境变量**（默认即可，无需修改）
...
> **提示：** 数据备份只需复制 `/docker/super-note/data/super-note.db` 文件。可使用群晖 Hyper Backup 定期备份该目录。也可通过 API `/api/backups` 进行在线备份管理。
...
1. **导入镜像**
   - 在电脑上构建并导出镜像（同群晖步骤）
   - 绿联 NAS → Docker → 镜像管理 → 本地导入 `super-note.tar`

2. **创建容器**
   - 选择 `super-note` 镜像 → 创建容器
   - **网络**：选择 bridge 模式
   - **端口映射**：主机 `3001` → 容器 `3001`
   - **存储映射**：
     - 主机路径：`/mnt/user/appdata/super-note/data`（或自定义路径，如绿联默认的 `共享文件夹/docker/super-note/data`）
...
>
> **验证方法**：容器启动几秒后进入绿联 Docker → 容器 → 终端，执行 `ls -la /app/data`，应能看到 `.jwt_secret`、`super-note.db`、`super-note.db-shm`、`super-note.db-wal` 四个文件；同时 NAS 文件管理器里挂载目录也应看到同样的文件。
>
> **备选方案**：如果你因特殊原因必须把容器目录保留为 `/data`，请额外在"环境变量"里配置 `SUPER_DATA_DIR=/data` 和 `DB_PATH=/data/super-note.db`，两者缺一不可。
...
1. **下载 .fpk**
   - 从 [GitHub Releases](https://github.com/cropflre/super-note/releases) 下载最新的 `super-note-<version>.fpk`
...
1. **导入镜像**
   - 在电脑上构建并导出镜像（同群晖步骤）
   - 飞牛 fnOS → Docker → 镜像 → 导入 `super-note.tar`

2. **创建容器**
   - 选择 `super-note` 镜像 → 创建容器
   - **端口映射**：主机 `3001` → 容器 `3001`
   - **卷映射**：
     - 主机路径：`/vol1/docker/super-note/data`（根据实际存储卷调整）
     - 容器路径：`/app/data`
   - **重启策略**：除非手动停止
...
1. **导入镜像**
   - 在电脑上构建并导出镜像（同群晖步骤）
   - Container Station → 映像档 → 导入 → 选择 `super-note.tar`

2. **创建容器**
   - 选择 `super-note` 映像 → 创建
   - **网络设置**：NAT 模式，端口映射 `3001` → `3001`
   - **共享文件夹**：
     - 主机路径：`/share/Container/super-note/data`
     - 容器路径：`/app/data`
   - **其他**：勾选"自动重新启动"
...
1. **导入镜像**
   - 在电脑上构建并导出镜像（同群晖步骤）
   - 极空间 → Docker → 镜像 → 本地镜像 → 导入 `super-note.tar`

2. **创建容器**
   - 选择 `super-note` 镜像 → 创建容器
   - **端口映射**：本地 `3001` → 容器 `3001`
   - **路径映射**：
     - 本地路径：选择一个文件夹（如 `极空间/docker/super-note/data`）
     - 容器路径：`/app/data`
   - **重启策略**：自动重启
...
# 产物：super-note-arm64.tar

# 3. 把 tar 文件传到板子（scp / U 盘都行），然后在板子上：
docker load -i super-note-arm64.tar
docker run -d \
  --name super-note \
  --restart unless-stopped \
  -p 3001:3001 \
  -v super-note-data:/app/data \
  cropflre/super-note:arm64
```
...
- **数据备份**：支持两种方式 — 直接备份 `super-note.db` 文件，或通过 API `/api/backups` 在线创建/下载备份
