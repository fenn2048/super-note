#!/usr/bin/env node
/**
 * Super Note MCP Server
 * 
 * 让 Codex CLI / Claude Desktop / Cursor 等 AI 工具直接操作 Super Note 笔记系统。
 * 
 * 环境变量：
...
import { z } from "zod";
import { SuperApiClient } from "./api-client.js";
...
const api = new SuperApiClient(config);
...
server.tool(
  "super_list_notebooks",
  "获取 Super Note 中的所有笔记本列表（支持树形结构），返回每个笔记本的 id、名称、图标、颜色和笔记数量",
  {},
  async () => {
    try {
...
server.tool(
  "super_create_notebook",
  "在 Super Note 中创建一个新笔记本",
  {
...
server.tool(
  "super_list_notes",
  "获取 Super Note 中的笔记列表。可按笔记本、标签、收藏状态、日期范围等筛选。返回笔记概要（不含完整内容）",
  {
...
server.tool(
  "super_read_note",
  "读取 Super Note 中指定笔记的完整内容（包括标题、正文、标签等全部信息）",
  {
...
server.tool(
  "super_create_note",
  "在 Super Note 中创建一篇新笔记。需要指定目标笔记本 ID",
  {
...
server.tool(
  "super_delete_note",
  "将 Super Note 中指定笔记移入回收站（软删除），或从回收站永久删除",
  {
    noteId: z.string().describe("笔记 ID"),
...
server.tool(
  "super_search",
  "在 Super Note 中全文搜索笔记。使用 FTS5 全文索引，支持模糊匹配，返回匹配的笔记摘要和高亮片段",
  {
...
server.tool(
  "super_list_tags",
  "获取 Super Note 中的所有标签列表，包含每个标签的颜色和关联笔记数",
  {},
  async () => {
    try {
...
server.tool(
  "super_manage_tags",
  "管理 Super Note 中笔记的标签：创建标签、给笔记添加标签、或移除笔记标签",
  {
...
server.tool(
  "super_ai_ask",
  "向 Super Note 知识库提问。系统会自动检索相关笔记内容，结合 AI 生成回答，并标注信息来源",
  {
...
server.tool(
  "super_knowledge_stats",
  "获取 Super Note 知识库的统计信息，包括笔记数、笔记本数、标签数、FTS 索引状态等",
  {},
  async () => {
    try {
...
server.tool(
  "super_list_plugins",
  "获取 Super Note 中已加载的插件列表，每个插件声明了它支持的能力（action）",
  {},
  async () => {
    try {
...
server.tool(
  "super_execute_plugin",
  "执行 Super Note 中指定名称的插件，传入参数并获取处理结果",
  {
...
server.tool(
  "super_list_webhooks",
  "获取 Super Note 中已配置的 Webhook 列表",
  {},
  async () => {
    try {
...
server.tool(
  "super_create_webhook",
  "在 Super Note 中创建新的 Webhook，当指定事件发生时自动推送 HTTP 通知",
  {
...
server.tool(
  "super_audit_stats",
  "获取 Super Note 的操作审计统计（按分类、级别统计，以及最近操作记录）",
  {},
  async () => {
    try {
...
server.tool(
  "super_create_backup",
  "创建 Super Note 数据备份。db-only 仅备份数据库，full 备份所有数据",
  {
...
async function main() {
  ...
  console.error(`🚀 Super Note MCP Server 已启动`);
